    ...
    for (const { patterns, entityType, blockType, special, label } of QUERY_PATTERNS) {
      for (const pattern of patterns) {
        if (pattern.test(lower)) {
          ...
          if (entityType) {
            const results2 = contentGraph.getEntities(entityType);
            return { type: entityType, label, results: results2, query, count: results2.length };
          }
          if (blockType) {
            const results2 = contentGraph.getBlocks(blockType);
            return { type: blockType, label, results: results2, query, count: results2.length };
          }
        }
      }
    }
...
`;
  ...
  async function saveToCache(pdfBuffer, analysisData, options = {}) {
    const { ttl = 7 * 24 * 60 * 60 * 1e3 } = options;
    try {
  ...
  async function loadFromCache(pdfBuffer) {
    try {
      const hash = await hashBuffer(pdfBuffer);
      const db = await openDB();
      return new Promise((resolve, reject) => {
