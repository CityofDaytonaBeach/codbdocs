var CodbDocs = (() => {
  ...
  __export(index_exports, {
    ...
    extractXfaFormFields: () => extractXfaFormFields,
    ...
    parseAcrobatFormRules: () => parseAcrobatFormRules,
  ...
  function parseAcrobatFormRules(actions) {
...
${p.text}`).join("\n\n");
  ...
  async function load(source, options = {}) {
    const pdfjsLib2 = getPdfjs4();
    let data;
    let sourceBytes4 = null;
    if (typeof source === "string") {
      data = { url: source };
    } else if (source instanceof ArrayBuffer) {
      sourceBytes4 = new Uint8Array(source.slice(0));
      data = { data: sourceBytes4.slice(0).buffer };
    } else if (source instanceof Uint8Array) {
      sourceBytes4 = source.slice(0);
      data = { data: sourceBytes4.slice(0).buffer };
    } else if (source && typeof source.arrayBuffer === "function") {
      sourceBytes4 = new Uint8Array(await source.arrayBuffer());
      data = { data: sourceBytes4.slice(0).buffer };
    } else {
      throw new Error("[codbdocs] Unsupported source. Pass a File, Blob, ArrayBuffer, Uint8Array, or URL string.");
    }
    const pdf = await pdfjsLib2.getDocument({ ...data, enableXfa: options.enableXfa !== false }).promise;
    return new CodbDoc(pdf, sourceBytes4);
  }
  var CodbDoc = class {
    constructor(pdf, sourceBytes4 = null) {
      this._pdf = pdf;
      this._sourceBytes = sourceBytes4;
        ...
        for (let num2 = 1; num2 <= this.pageCount; num2++) {
          ...
          if (this.isPureXfa && typeof page.getXfa === "function") {
            try {
              ...
              for (const field of extractXfaFormFields(irPage.xfa, num2)) {
                const object = registerFormField(ir, `page_${num2}`, field, num2);
...
${p.text}`).join("\n\n")
  ...
  function exportFidelityHTML(graph, options = {}) {
    ...
    const sourceBytes4 = graph?._sourceBytes || graph?._doc?._sourceBytes || null;
    const originalPdfSrc = options.originalPdfSrc || (options.includeOriginal !== false && sourceBytes4 ? `data:application/pdf;base64,${bytesToBase644(sourceBytes4)}` : void 0);
